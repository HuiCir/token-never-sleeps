...
TNS can import plans generated by Plan mode and convert them into tracked sections.
...
Or use the skill inside Claude:
...
TNS recognizes these heading patterns as section boundaries:
...
Use `--merge` to preserve existing section status when re-importing:

```bash
python3 scripts/tns_runner.py plan-import --config tns_config.json --plan-file plan.md --merge
```

This keeps `done` and `blocked` sections from previous runs while updating `pending` sections.

### Workflow Example

1. Generate a plan with Claude Plan mode
2. Save the plan output to `plan.md`
3. Import it: `/plan-import plan.md`
4. Start TNS: `/tns-start run --config tns_config.json`

Each section from the plan becomes a tracked TNS section that executor/verifier can process independently.
